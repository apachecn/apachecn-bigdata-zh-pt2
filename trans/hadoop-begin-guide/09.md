...
在[第八章](08.html "Chapter 8. A Relational View on Data with Hive")，*与配置单元*的数据关系视图中，我们提到了配置单元并不支持所有常见的 SQL数据类型。 特别是`DATE`和`DATETIME`类型目前还没有实现，尽管它们确实作为已确定的配置单元问题存在；因此，希望将来会添加它们。 我们在本章早些时候看到了这一影响，这是我们第一次导入Hive。 尽管 MySQL 中的`start_date`列是`DATE`类型，但 Sqoop 导入标记了一个转换警告，并且配置单元中的结果列是`STRING`类型。
