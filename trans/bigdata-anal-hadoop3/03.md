...
MapReduce 是一个用于在 Hadoop 集群中计算大量数据的框架。MapReduce 使用Yarn来调度映射器和 Reduce 作为任务，使用容器。MapReduce 框架使您能够编写分布式应用程序，以可靠和容错的方式处理文件系统中的大量数据，例如 **Hadoop 分布式文件系统** ( **HDFS** )。当您想要使用 MapReduce 框架来处理数据时，它通过创建作业来工作，然后在框架上运行作业来执行所需的任务。MapReduce 作业通常通过将输入数据拆分到工作节点，以并行方式运行映射器任务来工作。
