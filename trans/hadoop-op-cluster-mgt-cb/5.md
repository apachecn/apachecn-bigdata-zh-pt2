...
Hadoop NameNode将元数据保存在主内存中。 当 HDFS 命名空间变大时，主内存可能成为集群的瓶颈。 在适用于 MRv2 的 Hadoop 中引入了 HDFS 联合。 它通过利用多个独立 NameNode 的容量来增加 NameNode 容量和吞吐量，每个 NameNode 托管或管理 HDFS 命名空间的一部分。
...
将 NameNode ID添加到`$HADOOP_CONF_DIR/namenode_exclude.txt`文件中。 例如，如果我们想要从集群中停用`namenode1`，文件的内容应该是：
...
与旧的 Hadoop版本类似，平衡器用于平衡集群上的数据块。 在 HDFS 联合 Hadoop 集群上，我们可以使用以下命令运行平衡器：
