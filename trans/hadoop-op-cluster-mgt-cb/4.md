...
从节点上的 TaskTracker定期联系主节点上的 JobTracker。 两次连续接触通信之间的间隔称为**心跳**。 更频繁的心跳配置可能会给集群带来更高的负载。 心跳属性的值应该根据集群的大小进行设置。
...
与CapacityScheduler 类似，**Fair Scheduler**设计用于在多用户环境中实施集群资源的公平共享。 在本食谱中，我们将概述为 Hadoop 集群配置 Fair Scheduler 的步骤。
...
Hadoop Fair Scheduler以这样一种方式调度作业，即所有作业都可以获得相等份额的计算资源。 作业通过调度池进行组织。 可以为每个 Hadoop 用户配置一个池。 如果未配置用户的池，将使用默认池。 池指定用户可以在集群上共享的资源量，例如映射插槽数、减少插槽数、正在运行的作业总数等。
