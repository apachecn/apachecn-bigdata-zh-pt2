...
*   `AvroSerDe`：这是内置的 SerDe，可以读取和写入蜂窝表中的 AVRO(参见[http://avro.apache.org/](http://avro.apache.org/))数据。 AVRO 是一个远程过程调用和数据序列化框架。 从配置单元 v0.14.0 开始，只需通过三种方式将文件格式指定为`AVRO`，即可创建 Avro 支持的表格：
