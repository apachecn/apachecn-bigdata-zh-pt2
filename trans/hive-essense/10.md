...
作为一个通用的数据引擎，Apache Spark 可以与Hive紧密结合。 Spark SQL 支持 HQL 的一个子集，可以利用`Hive metastore`在配置单元中写入或查询数据。 这种方法也称为蜂巢上的火花。 要配置 Spark，请使用配置单元`metastore`，您只需将`hive-site.xml`复制到`${SPARK_HOME}/conf`目录。 之后，运行 Spark`spark-sql`命令将进入 Spark SQL 交互环境，在该环境中可以编写 SQL 来查询配置单元表。
