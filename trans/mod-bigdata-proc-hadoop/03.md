...
另一个资源协商者 ( **Yarn**)分离资源管理、调度和处理组件。它有助于实现集群资源的 100%资源利用率。Yarn基于 Hadoop 调度策略管理集群的中央处理器和内存。Yarn支持任何类型的应用，并不仅限于 MapReduce。它支持用任何类型的语言编写的应用，前提是可以在 Hadoop 集群上安装二进制文件。
