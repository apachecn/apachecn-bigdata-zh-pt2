...
为了使整个跨节点协调成为可能，名称节点依靠**动物园管理员**来跟踪哪个名称节点是主节点，哪个是备用节点，以防止两个节点都决定它们是主节点并开始将编辑日志写入日志节点的情况。动物园管理员是另一个Apache项目，是 CDH 的一部分。它提供分布式协调服务，并且在许多不同的节点需要共享状态、锁定信息或任何其他数据信息时非常有用。你可以在[http://zookeeper.apache.org](http://zookeeper.apache.org)找到更多关于动物园管理员的信息。名称节点高可用性难题的最后一块是动物园管理员故障转移控制器。ZKFC 是一个守护程序，它在主名称节点和备用名称节点上运行，检查它们的运行状况和状态，并在需要时启动到备用名称节点的故障转移。在处理名称节点故障时，集群管理员有两种选择:
