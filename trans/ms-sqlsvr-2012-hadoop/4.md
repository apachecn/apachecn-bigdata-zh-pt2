...
由于配置单元表仅通过 ODBC 公开，因此没有直接方法将 Analysis Services 数据库连接到 Hadoop，因为 Analysis Services只能连接到符合 OLE DB 的数据源。 要解决此问题，唯一的方法是使用配置单元 ODBC 驱动在 SQL Server实例中创建一个链接服务器，并通过 OLE DB for ODBC 使用它。 我们将重用在前面几章中创建的 DSN**HadoopOnLinux**来连接到配置单元。
