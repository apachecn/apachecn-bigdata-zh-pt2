...
该驱动程序有两种版本，64 位和 32 位，因此请确保根据客户端应用的位配置下载并安装适当的驱动程序。 在我的例子中，因为我将使用来自 32 位应用的驱动程序作为 Visual Studio，所以我使用了 32 位风格的驱动程序。 驱动程序安装完成后，您可以通过检查 ODBC 数据管理器的驱动程序列表中是否存在配置单元 ODBC 驱动程序来确认安装状态，如以下屏幕截图所示：
