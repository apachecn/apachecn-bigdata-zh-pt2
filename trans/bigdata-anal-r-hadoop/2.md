...
*   长期运行的缩减任务无法完成，因为它们的资源利用率很低，如果缩减任务花费太多时间来完成并且失败，或者如果没有其他可用的缩减槽来重新计划它(这可以通过Yarn来解决)。
...
Hadoop MapReduce 可用的两个应用编程接口是:新的(Hadoop 1.x 和 2.x)和旧的 Hadoop (0.20)。Yarn是下一代 Hadoop MapReduce 和新的 Apache Hadoop 子项目，已为 Hadoop 资源管理发布。
