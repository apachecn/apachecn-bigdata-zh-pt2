...
3.  用`conf/*-site.xml`格式更新Hadoop配置文件。
...
**Pig** 是一个基于 Hadoop 的开源平台，通过自己的类似 SQL 的语言:Pig Latin 来分析大规模数据集。这为大量复杂的数据并行计算提供了一个简单的操作和编程接口。这也更容易开发；它更加优化和可扩展。ApachePIG已经被雅虎开发出来了！。目前，雅虎！和推特是小PIG的主要用户。
