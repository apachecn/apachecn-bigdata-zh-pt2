...
在前面的示例中，编译器生成的 MapReduce 作业将运行 10 个并发 Reduce 任务。 PIG对使用多少减速机有一个试探性的估计。 全局强制执行 Reduce 任务数量的另一种方法是使用`set default_parallel <n>`命令。
...
在本例中，我们使用`CONCAT`将一个(不是很随机的)SALT 字符串附加到个人数据。 然后，我们使用 DataFu 的`SHA`函数生成加盐 ID 的散列。 `SHA`函数要求其输入参数为非空。 我们使用`if-then-else`语句强制执行此条件。 在PIG拉丁语中，这表示为`<condition is true> ? <true branch> : <false branch>`。 如果字符串为空，则返回`NULL`，如果不为空，则返回加盐的散列。 为了使代码更具可读性，我们对 tweet JSON 字段使用别名，并在`GENERATE`块中引用它们。
