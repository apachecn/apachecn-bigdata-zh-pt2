...
为了在Yarn上部署 Spark，需要构建一个临时合并 JAR。 Spark 在 ResourceManager 中启动独立部署的集群的一个实例。 Cloudera 和 MapR 都将 Spark on Sink 作为其软件分发的一部分。 在撰写本文时，Spark 已经作为技术预览版提供给 Hortonworks 的 HDP([http://hortonworks.com/hadoop/spark/](http://hortonworks.com/hadoop/spark/))。
